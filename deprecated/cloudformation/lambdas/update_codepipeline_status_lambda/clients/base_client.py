...
class BaseAPIClass:
    ...
    def _generate_basic_auth(self) -> HTTPBasicAuth:
    ...
    def get(self, path: str, param: dict = None, headers: dict = None) -> Tuple[int, dict]:
        """
        ...
        auth = self._generate_basic_auth()
        headers = self._generate_headers(headers)
        response = requests.get(url=url, headers=headers, params=param, auth=auth)
        return self._handle_response(response)

    def post(self, path: str, data: Union[dict, bytes, list, str] = None, headers: dict = None) -> Tuple[int, dict]:
        """
